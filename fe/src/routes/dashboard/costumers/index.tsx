  ...
  return (
    <div className="space-y-8 p-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-gray-500">
            View profiles, orders, and loyalty for every shopper.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="border-b border-emerald-50 bg-gray-50/50">
              <tr>
                {['Customer', 'Email', 'Location', 'Orders', 'Tier', ''].map(
                  (h) => (
                    <th
                      key={h || 'action'}
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="text-sm font-bold text-[#173a40]">
                        {c.name}
                      </span>
                    </div>
                  </td>
      ...
      <div className="flex items-center gap-3 rounded-[24px] border border-emerald-100 bg-emerald-50/50 px-5 py-4 text-sm text-gray-600">
        ...
        <span>
          Showing <strong className="text-[#173a40]">{customers.length}</strong>{' '}
          customers. Open a row to see full details, purchase history, and
          perks.
        </span>
      </div>
    </div>
